...
area_func = @(x, y) 0;
for i = 1:length(x)-1
    area_func = area_func + 0.5*(y(i)+y(i+1))*(x(i+1)-x(i));
end
x_values = x_func(theta_range);
y_values = y_func(theta_range);
area = sum(area_func(x_values, y_values));
...
while abs(a - a_ref) > tolerance
    ...
    area_func = @(x, y) area + area_val; % Add new area to previous area
    a = area_func(x_values, y_values);
