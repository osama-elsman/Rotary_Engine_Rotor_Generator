...
R = 76.7075015111693;
    ...
    function calculate_R(e, a_ref, plot_panel, fig)
        ...
        a_cf = 1.07426688351018; % Area Correction Factor
        ...
        area_func = @(x, y, R) a_cf * sum(area_val);
        x_values = x_func(theta_range, R_0);
        y_values = y_func(theta_range, R_0);
        area = a_cf * sum(area_func(x_values, y_values, R_0));
        ...
        while abs(a - a_ref) > tolerance
            ...
            area_func = @(x, y, R) area + area_func_R(x, y)*(R/R_0); % Define updated area function
            a = a_cf * (area + area_val*(R/R_0)); % Calculate a for current R
